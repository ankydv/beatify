...
const MusicList = ({ dataset, isQueue, onPress, onEndReached }) => {
  ...
  if (!onPress) {
    onPress = (song) => {
      ...
      if (!isQueue) navigation.navigate("player");
    };
  }
  ...
  const getThumbUrl = (song) => {
    ...
    } else if (song.thumbnails) return song.thumbnails[0].url;
    else return null;
  };

  const renderItem = ({ item: song, index }) => (
    <MusicListItem
